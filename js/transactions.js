...
*/	
	...
	var data_file  = "data/txns/3000_part1_txns_latlong.json"; // "data/txns/100_txns_latlong.json "; //
	var data_file2 = "data/txns/3000_part2-gap_txns_latlong.json";
	...
	function update_transaction_data(new_txns) {
		...
		if (new_txns[0].time.getTime() >= 
			...
			// have new txns and old txns
			// determine how many
			// new.length = 100, limit = 400, keep = limit-new.length
			// 		var old_keep = keep > 0 ? old.slice(-(400-100)) : [];
			//      var new_keep = new_txns.slice(-limit)
			//
			// compute hist values with both old_keep.concat(new_keep)
			//  	--> need to get the fill index out
			//			include in 
			...
			//var old_timeline_cts = timeline_cts.slice(0);   // shallow copies
			...
			// only the new transactions because the arcs reference these values
	...
	function get_fillto_idx(old_txns, timeline_cts) {
		...
		for (var bin_idx = 0; bin_idx < timeline_cts.length; bin_idx++) {
			...
			if (prev_maxtime > timeline_cts[bin_idx].x.getTime()) {
				fillto_idx = bin_idx;
				//console.log("prev_maxtime (" + prev_maxtime + ") > currbin x (" + timeline_cts[bin_idx].x.getTime() + ")");

			} else { // max_ct falls in this bin so cannot fill it
				//console.log("prev_maxtime (" + prev_maxtime + ") <= currbin (" + bin_idx + ") x (" + timeline_cts[bin_idx].x.getTime() + ")");
			}
		}
		return fillto_idx;
	}
